...
if page == "🏠 Command Center":
    ...
    st.caption("✨ State names are displayed in bright white with high-contrast labeling.")
    ...
    if india_geojson:
        ...
        # Dark underlay makes bright state names readable on every map shade
        fig_map.add_trace(
            go.Scattermapbox(
        ...
        fig_map.add_trace(
            go.Scattermapbox(
                ...
                # Bright white state names with a dark outline/shadow effect
                # is simulated by using a second text trace below.
    ...
    # VISIBLE STATE SELECTION FOR VIEWERS
    state_list = ["All States"] + sorted(df["State"].dropna().unique().tolist())

    st.markdown(
        '''
        <div class="selection-panel">
            <div class="selection-title">📍 State Details</div>
            <div class="selection-help">
                Choose a state to view its infrastructure projects, pending works,
...
# ============================================================
    ...
    st.markdown(
        '''
        <div class="selection-panel">
            <div class="selection-title">🏗️ Project Filters</div>
            <div class="selection-help">
                Select a sector below to filter infrastructure projects.
            </div>
        </div>
        ''',
        unsafe_allow_html=True,
    ...
    search = st.text_input("🔎 Search project", placeholder="Type a project name...")
    sectors = ["All Sectors"] + sorted(df["Sector"].dropna().unique().tolist())
    selected_sector = st.selectbox(
        "🏗️ Select Sector",
        sectors,
        key="project_explorer_sector",
    )
...
"""
    ...
    st.divider()
    st.caption("NAV-NIRMAAN AI Assistant • Prototype Intelligence Engine")